...
# physical degradation of detritus to biologically oxidizable Carbon (BOC), from detritus 
...
# detritus at time step is the result of Detritus from the previous time step, plus detritus recharged from precipitation minus what is broken down to BOC, plus Import from freshly dead biomass (mortality). 
...
#  K_ac is temperature dependent, such as in Schmidt et al. (2018)
d_K_ac_per_temperature    = function(K_ac, lab_temp, GWTEMP_ti) {
  
  # change the rate by the factor by which it deviates from the rate measured at lab temperature 
  TEMP_related_to_lab_temperature = lab_temp-GWTEMP_ti
  K_ac_at_TEMP <- K_ac  / (TEMP_related_to_lab_temperature * 0.9696) #0.9696  is the factor in Schmidt et al. 2018 to calculate respective temperature in 1 degree steps
...
# multiplying microbial growth with yield and checking that microbial dry mass not negative
dBOC_MO_degradation_dt    = function(BOC_ti_minus_1,   rMO_BOC_uptake_per_day_at_TEMP,  K_ac_at_TEMP, MO_het_ti_minus_1, yield_ac, f_S_fauna, delta_t, growth_model_MO_type, CC_group_MO_g) {
  ...
  if(MO_het_ti <= 0){
    MO_het_ti <- .00000000001 #MO_het_ti_minus_1 artificially , microorganisms do NOT vanish - they hide from the grazing presure.  this step feeds the fauna artificially. so take a very small value
  }else{
    MO_het_ti <- MO_het_ti   }
...
dMO_fauna_uptake_dt      = function(MO_het_ti_minus_1,   rFauna_MO_uptake_per_day_at_TEMP,  K_MO_at_temp, Fauna_ti_minus_1, delta_t, growth_model_fauna_type, CC_group_fauna_g, yield_MO, Excretion , Mortality) {
  ...
  Fauna_ti_ <- Fauna_ti_minus_1 + fauna_growth - Excretion - Mortality
  
  # 
  if(Fauna_ti_ < 0){ 
    Fauna_ti_ <- .000000001# assumption: if fauna is starving to death, there might still be some immigration - which requires further process steps not implemented yet
  }else{
    Fauna_ti_ <- Fauna_ti_ 
  }
  
  return( list(fauna_growth, Fauna_ti_ )) #
